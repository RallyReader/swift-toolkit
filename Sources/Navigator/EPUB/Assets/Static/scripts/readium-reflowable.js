...
function DecorationGroup(groupId, groupName) {
  ...
  function layoutEnhanced(item, postMessage) {
    ...
    function positionElement(element, rect, boundingRect) {
      element.style.position = "absolute";
      (0,_utils__WEBPACK_IMPORTED_MODULE_1__.log)("style width: ".concat(style.width));
      (0,_utils__WEBPACK_IMPORTED_MODULE_1__.log)("rect left: ".concat(rect.left));
      if (style.width === "wrap") {
        element.style.width = "".concat(rect.width, "px");
        element.style.height = "".concat(rect.height, "px");
        element.style.left = "".concat(rect.left + xOffset, "px"); // Position within the page
        element.style.top = "".concat(rect.top + yOffset, "px");
      } else if (style.width === "viewport") {
        element.style.width = "".concat(viewportWidth, "px");
        element.style.height = "".concat(rect.height, "px");
    ...
    (0,_utils__WEBPACK_IMPORTED_MODULE_1__.log)("style layout: ".concat(style.layout));
    if (style.layout === "boxes") {
      let doNotMergeHorizontallyAlignedRects = true;
      let clientRects = (0,_rect__WEBPACK_IMPORTED_MODULE_0__.getClientRectsNoOverlap)(item.range, doNotMergeHorizontallyAlignedRects);
      clientRects = clientRects.sort((r1, r2) => {
...
function getCurrentSelectionText() {
  ...
  log("selection highlight: ".concat(highlight));
  let cleanHighlight = highlight.trim().replace(/\n/g, " ").replace(/\s\s+/g, " ");
  log("selection clean highlight: ".concat(cleanHighlight));
...
function getTextFrom(highlight, range) {
  ...
  let originalStart = _vendor_hypothesis_anchoring_text_range__WEBPACK_IMPORTED_MODULE_2__.TextRange.fromRange(range).relativeTo(document.body).start.offset;
  let originalEnd = _vendor_hypothesis_anchoring_text_range__WEBPACK_IMPORTED_MODULE_2__.TextRange.fromRange(range).relativeTo(document.body).end.offset;

  // Traverse original text and fullText at the same time to find the correct offsets
  for (let i = 0, j = 0; i < originalText.length && j < fullText.length; i++, j++) {
  ...
  let before = fullText.slice(Math.max(0, rangeStartOffset - snippetLength), rangeStartOffset);
  ...
  let after = fullText.slice(rangeEndOffset, Math.min(fullText.length, rangeEndOffset + snippetLength));
  ...
  log("before: ".concat(before));
  log("highlight: ".concat(highlight));
  log("after: ".concat(after));
...
function calculateHorizontalPageRanges() {
  ...
  function processTextContent(element, textContent) {
    ...
    while (wordBoundingRect.x + wordBoundingRect.width > (currentPage + 1) * pageWidth && words.length > 0) {
      ...
      if (removedWord === " ") {
        ...
        try {
          let anchor = new _vendor_hypothesis_anchoring_types__WEBPACK_IMPORTED_MODULE_0__.TextQuoteAnchor(element, removedWord, {
            prefix: words.join(""),
            // Join without adding any additional characters
          ...
          if (wordBoundingRect.x + wordBoundingRect.width > (currentPage + 1) * pageWidth) {
            removedText = removedWord + removedText;
          }
...
function rangeFromCachedLocator(locator) {
  ...
  while (startIndex < entireText.length) {
    const highlightIndex = entireText.indexOf(locator.text.highlight, startIndex);
    if (highlightIndex === -1) {
      break; // No more occurrences of highlight text
    }

    const beforeText = locator.text.before ? entireText.slice(Math.max(0, highlightIndex - locator.text.before.length), highlightIndex) : "";
    const afterText = locator.text.after ? entireText.slice(highlightIndex + locator.text.highlight.length, highlightIndex + locator.text.highlight.length + locator.text.after.length) : "";
    const beforeTextMatches = !locator.text.before || locator.text.before.endsWith(beforeText);
    const afterTextMatches = !locator.text.after || locator.text.after.startsWith(afterText);
  ...
  let count = 0;
  let node;
  const nodeIterator = document.createNodeIterator(entireRange.commonAncestorContainer,
  // This should be a Document or DocumentFragment node
